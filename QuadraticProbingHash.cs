...
public class QuadraticProbingHash<T> : IHashing<T> where T : IRegistro<T>, new()
{
    ...
    public QuadraticProbingHash() : this(TAMANHO_PADRAO) { }

    public QuadraticProbingHash(int tamanhoDesejado)
    {
        // IMPORTANTE: Para sondagem quadrática funcionar corretamente,
        // o tamanho da tabela deve ser primo
    ...
    {
        if (totalElementos >= tabelaDeHash.Length * 0.5) // Fator de carga menor para sondagem quadrática
            return false;

        int posicaoInicial = Hash(novoDado.Chave);
        
        // Sondagem Quadrática: incrementa com i²
        for (int i = 0; i < tabelaDeHash.Length; i++)
        {
            int posicao = (posicaoInicial + i * i) % tabelaDeHash.Length;
    ...
    {
        int posicaoInicial = Hash(dado.Chave);
        
        // Sondagem Quadrática para busca
        for (int i = 0; i < tabelaDeHash.Length; i++)
        {
            onde = (posicaoInicial + i * i) % tabelaDeHash.Length;
